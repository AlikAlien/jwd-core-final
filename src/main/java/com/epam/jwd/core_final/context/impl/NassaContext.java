...
import com.epam.jwd.core_final.sql.Connector;
import com.epam.jwd.core_final.sql.SQLTable;
    ...
    public <T extends BaseEntity> Collection<T> retrieveBaseEntityList(Class<T> tClass, String inputFile) throws IOException {
        ...
        //try {
            if (tClass == CrewMember.class) {
                ...
                crewMembers = (Collection<CrewMember>) Connector.CONNECTOR.sql2array(SQLTable.CREWMEMBERS, SQLTable.getSqlCrewMember());
            }
            if (tClass == Spaceship.class) {
                ...
                spaceships = (Collection<Spaceship>) Connector.CONNECTOR.sql2array(SQLTable.SPACESHIPS, SQLTable.getSqlSpaceship());
            }

            if (tClass == Route.class) {
                //ReadEntity readRoute = new ReadRoute();
                //routes = (Collection<Route>) readRoute.readEntityList(filePath);
                routes = (Collection<Route>) Connector.CONNECTOR.sql2array(SQLTable.ROUTES, SQLTable.getSqlRoutes());
            }
